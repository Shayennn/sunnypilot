...
def setup_interfaces(CI, CP: structs.CarParams, CP_SP: structs.CarParamsSP,
  ...
  _initialize_tesla(CP, CP_SP, params_dict)
...
def _initialize_tesla(CP: structs.CarParams, CP_SP: structs.CarParamsSP,
                      params_dict: dict[str, str]) -> None:
  if CP.brand == 'tesla':
    ...
    # Tesla's profile is a stock DAS setting and has no effect when openpilot
    # replaces longitudinal control. Keep the CAN path closed in that mode.
    speed_profile = int(params_dict.get("TeslaSpeedProfile", 0)) == 1
    if speed_profile and not CP.openpilotLongitudinalControl:
      CP_SP.flags |= TeslaFlagsSP.SPEED_PROFILE.value
      CP_SP.safetyParam |= TeslaSafetyFlagsSP.SPEED_PROFILE
